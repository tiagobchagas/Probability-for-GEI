...
x_lab <- "Adjusted means Yield"
y_lab <- "Density"
#x_title <- ""
x_title <- "A: Observed against generated data Yield" 
(p1A <- ggplot(data_fig1, aes(x = y, colour = type)) + 
                ...
                ylim(0, 0.00035) + labs(x = x_lab, y = y_lab, title = x_title)) 
...
rm(g_postChecks)
# PLots
x_lab <- "Genetic values Yield"
y_lab <- "Families derived from Cycle III Recurrent Selection"
x_title <- "B: 95 % Highest posterior density interval Yield"
levels(df$H)
# Plot 
...
for (s in 1:nrow(g_post)){
        top20 = order(g_post[s,], decreasing = T)[1:ceiling(0.2*length(g_post[s,]))]
        ind_post[s,top20]  = 1
        ind_post[s,-top20] = 0
}
...
y_lab <- "Probability of superior performance Yield"
x_title <- "C: Marginal probability of superior performance Yield"
...
index <- index[order(index$g_post, decreasing = T),]
index[1:5,]
...
x_lab <- "Families derived Cycle III Recurrent Selection"
y_lab <- "Families derived Cycle III Recurrent Selection Yield"
x_title <- "D: Pairwise probability of superior performance Yield"
...
# Rordering matrix by the mean probability:
index = order(rowMeans(prob_stab_ggm), decreasing = T)
...
x_title <- "A: Bayesian Eskridge Risk index Yield"
dev.off()
...
x_lab <- 'Families derived Cycle III Recurrent Selection'
y_lab <- 'Probabilities'
x_title <- "B: Joint probability of superior performance and stability Yield"
